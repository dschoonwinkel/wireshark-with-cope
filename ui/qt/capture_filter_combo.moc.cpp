/****************************************************************************
** Meta object code from reading C++ file 'capture_filter_combo.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.2.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "capture_filter_combo.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'capture_filter_combo.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.2.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_CaptureFilterCombo_t {
    QByteArrayData data[10];
    char stringdata[173];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_CaptureFilterCombo_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_CaptureFilterCombo_t qt_meta_stringdata_CaptureFilterCombo = {
    {
QT_MOC_LITERAL(0, 0, 18),
QT_MOC_LITERAL(1, 19, 17),
QT_MOC_LITERAL(2, 37, 0),
QT_MOC_LITERAL(3, 38, 22),
QT_MOC_LITERAL(4, 61, 21),
QT_MOC_LITERAL(5, 83, 26),
QT_MOC_LITERAL(6, 110, 5),
QT_MOC_LITERAL(7, 116, 12),
QT_MOC_LITERAL(8, 129, 24),
QT_MOC_LITERAL(9, 154, 17)
    },
    "CaptureFilterCombo\0interfacesChanged\0"
    "\0pushFilterSyntaxStatus\0popFilterSyntaxStatus\0"
    "captureFilterSyntaxChanged\0valid\0"
    "startCapture\0saveAndRebuildFilterList\0"
    "rebuildFilterList\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_CaptureFilterCombo[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       7,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       5,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   49,    2, 0x06,
       3,    1,   50,    2, 0x06,
       4,    0,   53,    2, 0x06,
       5,    1,   54,    2, 0x06,
       7,    0,   57,    2, 0x06,

 // slots: name, argc, parameters, tag, flags
       8,    0,   58,    2, 0x08,
       9,    0,   59,    2, 0x08,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString,    2,
    QMetaType::Void,
    QMetaType::Void, QMetaType::Bool,    6,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void,

       0        // eod
};

void CaptureFilterCombo::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        CaptureFilterCombo *_t = static_cast<CaptureFilterCombo *>(_o);
        switch (_id) {
        case 0: _t->interfacesChanged(); break;
        case 1: _t->pushFilterSyntaxStatus((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 2: _t->popFilterSyntaxStatus(); break;
        case 3: _t->captureFilterSyntaxChanged((*reinterpret_cast< bool(*)>(_a[1]))); break;
        case 4: _t->startCapture(); break;
        case 5: _t->saveAndRebuildFilterList(); break;
        case 6: _t->rebuildFilterList(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (CaptureFilterCombo::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&CaptureFilterCombo::interfacesChanged)) {
                *result = 0;
            }
        }
        {
            typedef void (CaptureFilterCombo::*_t)(const QString & );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&CaptureFilterCombo::pushFilterSyntaxStatus)) {
                *result = 1;
            }
        }
        {
            typedef void (CaptureFilterCombo::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&CaptureFilterCombo::popFilterSyntaxStatus)) {
                *result = 2;
            }
        }
        {
            typedef void (CaptureFilterCombo::*_t)(bool );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&CaptureFilterCombo::captureFilterSyntaxChanged)) {
                *result = 3;
            }
        }
        {
            typedef void (CaptureFilterCombo::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&CaptureFilterCombo::startCapture)) {
                *result = 4;
            }
        }
    }
}

const QMetaObject CaptureFilterCombo::staticMetaObject = {
    { &QComboBox::staticMetaObject, qt_meta_stringdata_CaptureFilterCombo.data,
      qt_meta_data_CaptureFilterCombo,  qt_static_metacall, 0, 0}
};


const QMetaObject *CaptureFilterCombo::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *CaptureFilterCombo::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_CaptureFilterCombo.stringdata))
        return static_cast<void*>(const_cast< CaptureFilterCombo*>(this));
    return QComboBox::qt_metacast(_clname);
}

int CaptureFilterCombo::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QComboBox::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 7)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 7;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 7)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 7;
    }
    return _id;
}

// SIGNAL 0
void CaptureFilterCombo::interfacesChanged()
{
    QMetaObject::activate(this, &staticMetaObject, 0, 0);
}

// SIGNAL 1
void CaptureFilterCombo::pushFilterSyntaxStatus(const QString & _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void CaptureFilterCombo::popFilterSyntaxStatus()
{
    QMetaObject::activate(this, &staticMetaObject, 2, 0);
}

// SIGNAL 3
void CaptureFilterCombo::captureFilterSyntaxChanged(bool _t1)
{
    void *_a[] = { 0, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 3, _a);
}

// SIGNAL 4
void CaptureFilterCombo::startCapture()
{
    QMetaObject::activate(this, &staticMetaObject, 4, 0);
}
QT_END_MOC_NAMESPACE
